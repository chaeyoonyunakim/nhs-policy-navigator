...
The UI also surfaces live news/publication cards and a **"This query"** panel that explains the agent's choices for the latest answer in plain English — query type, sources searched, and retrieval strategy. The running total of queries processed sits on the Query Router digest. A dedicated **Previous Queries** tab browses the full query history (paginated, 10 per page, filterable by care setting / professional group), and any past question can be copied for reuse.
